...
char *  cmd_to_string( unsigned int cmd)
{
    ...
    case GF_IOC_TOUCH_ENABLE_MASK:
        cmd_string = "GF_IOC_TOUCH_ENABLE_MASK";
        break;
...
static void del_FpMaskTouch_Timer(struct gf_dev *gf_dev)
{
	del_timer(&gf_dev->FpMaskTouch_Timer);
	gf_dev->FpTimer_expires = 0;
	g_FP_Disable_Touch = false;
	printk("gf: Del Disable touch timer\n");
	return;
}
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case GF_IOC_TOUCH_ENABLE_MASK:
        ...
        //pr_info(" GF_IOC_TOUCH_ENABLE_MASK !\n");
        break;
    case GF_IOC_TOUCH_DISABLE_MASK:
...
#endif
{
	...
	if(g_Charger_mode)
	{
		pr_warn("gf: it is in charger mode,return the probe func.\n");
		return -1;
	}
