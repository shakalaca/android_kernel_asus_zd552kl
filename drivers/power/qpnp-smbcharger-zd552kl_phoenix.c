...
static int asus_battery_update_status(struct smbchg_chip *chip)
{
	int status = POWER_SUPPLY_STATUS_UNKNOWN;

	if (chip->usb_present == 1 && get_prop_batt_capacity(chip) ==100)
	{
		status = POWER_SUPPLY_STATUS_FULL;
	}
	else if (chip->usb_present == 0 && get_prop_batt_capacity(chip) ==100)
	{
		status = POWER_SUPPLY_STATUS_DISCHARGING;
	}
	else
...
static int smbchg_otg_regulator_enable(struct regulator_dev *rdev)
{
	...
	rc = smbchg_sec_masked_write(chip, chip->otg_base + SMBCHGL_OTG_OTG_ICFG,
		SMB_MASK(7, 0),chip->usb_connector_event ? 0 : 0x1);
	if (rc < 0) {
		pr_err("Couldn't configure set_OTG_ICFG rc=%d\n", rc);
	}

	rc = smbchg_masked_write(chip, chip->bat_if_base + CMD_CHG_REG,
			OTG_EN_BIT, chip->usb_connector_event ? 0 : OTG_EN_BIT);
	if (rc < 0)
		dev_err(chip->dev, "Couldn't enable OTG mode rc=%d\n", rc);
	else
		chip->otg_enable_time = ktime_get();
	pr_smb(PR_STATUS, "Enabling OTG Boost; usb_connector_event=%d\n", chip->usb_connector_event);
...
static void handle_usb_insertion(struct smbchg_chip *chip)
{
	...
	//liquid_flag = gpio_get_value(chip->vbus_low_impedance_irq);
	...
	printk("[CHARGE]%s do soft jeita after 0s\n",__FUNCTION__);
	cancel_delayed_work(&chip->asus_batt_temp_work);
	schedule_delayed_work(&chip->asus_batt_temp_work,0);
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		val->intval = asus_battery_update_status(chip);
...
static irqreturn_t usbid_change_handler(int irq, void *_chip)
{
	...
	if (otg_present){
		chip->usb_connector_event = gpio_get_value(chip->usb_thermal_irq);
		switch_set_state(&chip->usb_thermal, chip->usb_connector_event);
		pr_smb(PR_STATUS, "OTG detected; usb_connector_event=%d\n", chip->usb_connector_event);
	}
...
static void asus_adapter_detect_reset_work(struct smbchg_chip *chip)
{
	...
	rc = vote(chip->usb_icl_votable, ASUS_ADAPTER_VOTER, false, 0);
	if(rc < 0){
		pr_err("can not cancel set ASUS_ADAPTER_VOTER!\n");
	}
...
static void asus_adapter_read_adc_work(struct work_struct *work)
{
	...
	rc = vote(chip->usb_icl_votable, ASUS_ADAPTER_VOTER, true, 300);
	if(rc){
		pr_err("Couldn't vote for ASUS_ADAPTER_VOTER rc=%d\n", rc);
	}
	mdelay(60);
...
//	printk("[BAT][alarm]Jeita_Alarm triggered\n");
	if (!smb_charger_dev) {
		pr_err("driver not ready yet!\n");
		return ALARMTIMER_NORESTART;
	}

	if(is_usb_present(smb_charger_dev) == true)
	{
		wake_lock_timeout(&UsbCable_Lock, 2 * HZ);
		cancel_delayed_work(&smb_charger_dev->asus_batt_temp_work);
		schedule_delayed_work(&smb_charger_dev->asus_batt_temp_work,0);
	}
	return ALARMTIMER_NORESTART;
}
...
static ssize_t vbus_liquid_switch_name(struct switch_dev *sdev, char *buf)
{
	return sprintf(buf, "vbus_liquid\n");
}
...
static ssize_t usb_thermal_switch_name(struct switch_dev *sdev, char *buf)
{
	return sprintf(buf, "usb_connector\n");
}

static ssize_t usb_thermal_switch_state(struct switch_dev *sdev, char *buf)
{
	if(!smb_charger_dev){
		pr_info("charger is NULL!\n");
		return 0;
	}

	return sprintf(buf, "%d\n", smb_charger_dev->usb_connector_event);
}
...
void asus_vbus_low_impedance_det_work( struct work_struct *work)
{
	...
	//report uevent again when vbus flag is high
	if(chip->usb_present){
		switch_set_state(&chip->vbus_liquid, 1);
	}
...
void asus_set_usb_connector_work ( struct work_struct *work)
{
	...
	int otg_en = smbchg_otg_regulator_is_enable(chip->otg_vreg.rdev);
	...
	if (charge_en == 1) {
			...
			printk("[BAT][CHG] usb_temp_alert_interrupt,suspend charger,otg_en=%d\n", otg_en);
...
void USB_alert_detect_work(struct work_struct *work)
{
	...
	if (boot_completed_flag) {
		...
		if(g_Charger_mode==0){
			schedule_delayed_work(&chip->set_usb_connector_work, 0);
			schedule_delayed_work(&chip->vbus_low_impedance_det_work, 0);
		}
	} else {
...
//	INIT_DELAYED_WORK(&chip->wake_unlock_work, asus_wake_unlock_work);
	...
	chip->usb_thermal.print_name = usb_thermal_switch_name;
	chip->usb_thermal.print_state = usb_thermal_switch_state;
	...
	chip->vbus_liquid.print_name = vbus_liquid_switch_name;
