...
struct cm36656_info {
  ...
  int als_polling_delay;
	...
	int ps_autok_max;
	#endif
	uint8_t status_calling;//asus 20170606 status calling
	//asus alex_wang psensor autok---
	//uint16_t ls_cmd;
...
//disable touch when proximity is close in call +++
extern void focal_psensor_disable_touch(bool enable);
...
bool proximity_check_status(void){
   ...
   int ret,status;
   wake_lock(&lpi->ps_irq_wake_lock);
	...
	case ASUS_PSENSOR_SETCALI_DATA:
	  ...
	  //asus alex wang PS Calibration++++
	    ...
	    lpi->ps_crosstalk_diff = 0;
            ...
            //asus20170517 alex wang for psensor CSC autok-----
	     lpi->ps_autok_min = lpi->ps_crosstalk*3/100; //decided by EE
	     lpi->ps_autok_max = lpi->ps_crosstalk*150/100; //decided by EE
             //asus20170420 alex wang psensor autok++++
            #endif
...
static int cm36656_suspend(struct device *dev)
{
	...
	*/
	...
	lpi->ps_autok_enabled_before_suspend= lpi->ps_autok_enable;
	 if(status_calling){
	 	    if(lpi->ps_irq_flag == 0){	
	                 disable_irq_nosync(lpi->irq);
	         	 lpi->ps_suspend_irq_flag=1;
            	     }else{
            	        D("[cm36656] suspend irq wakelock\n");
	                wake_lock(&lpi->ps_irq_wake_lock);
	             }
