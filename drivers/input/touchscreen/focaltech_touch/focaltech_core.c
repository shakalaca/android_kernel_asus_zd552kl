...
//<ASUS-BSP Robert_He 20170509> release capsensor when suspend/resume ++++++
	press_touch = false;
//<ASUS-BSP Robert_He 20170509> release capsensor when suspend/resume ------
...
//<ASUS-BSP Robert_He 20170608> disable touch when proximity is close in call ++++++
void focal_psensor_disable_touch(bool enable)
{
	if(enable)
	{
		PROXIMITY_DISABLE_TOUCH= true;
		printk("[Touch] : proximity trigger disable touch !\n");
	} 
	else
	{
		PROXIMITY_DISABLE_TOUCH = false;
                                ...
                                unsigned long event, void *data)
{
    ...
    if (evdata && evdata->data && event == FB_EVENT_BLANK &&
        ...
        blank = evdata->data;
//<ASUS-BSP Robert_He 20170505> treat other frame buffer as powerdown for panel alwayson mode++++++
        if (*blank == FB_BLANK_UNBLANK)
            //fts_ts_resume(&fts_data->client->dev);
            queue_work(fts_data->tp_suspend_resume_wk,&fts_data->touch_resume_work);
        else if (*blank == FB_BLANK_POWERDOWN || *blank == FB_BLANK_NORMAL || *blank == FB_BLANK_VSYNC_SUSPEND)
            //fts_ts_suspend(&fts_data->client->dev);
...
{
	u8 cap_ver;
	cap_ver = asus_get_capsensor_fw_ver();
	return snprintf(buf,PAGE_SIZE,"%02x_%02x_%02x\n",
             asus_vendor_id_touch,asus_firmware_id_touch,cap_ver);
}
...
//<Focal driver-team libaojian 20170828> add irq_lock and mutex ------
	...
	wake_lock_init(&data->touch_wake_lock, WAKE_LOCK_SUSPEND, "focal_touch_wake_lock");
    if (err) {
...
//<ASUS-BSP Robert_He 20170525> add touch resume/suspend workqueue ++++++
	data->tp_suspend_resume_wk = create_singlethread_workqueue("fts_touch_wq");
	if (!data->tp_suspend_resume_wk)
	{
		FTS_ERROR("touch resume_suspend wk create fail");
		goto exit_create_singlethread;
	}
	INIT_WORK(&data->touch_suspend_work, fts_suspend_work);
	INIT_WORK(&data->touch_resume_work, fts_resume_work);
//<ASUS-BSP Robert_He 20170525> add touch resume/suspend workqueue ------
...
static int fts_ts_suspend(struct device *dev)
{
    ...
    FTS_FUNC_ENTER();
//<ASUS-BSP Robert_He 20170505> add esd check suspend/resume logic ++++++
	if (ESD_SUSPEND_TOUCH)
	{
		FTS_INFO("ESD check fail, should not suspend touch");
		return 0;
	}
//<ASUS-BSP Robert_He 20170505> add esd check suspend/resume logic ------
    if (data->suspended) {
	...
	wake_lock(&fts_wq_data->touch_wake_lock);
...
//<ASUS-BSP Robert_He 20170606> release each slot by reporting down and up ++++++
		fts_release_all_finger();
//<ASUS-BSP Robert_He 20170606> release each slot by reporting down and up ------
        ...
        FTS_FUNC_EXIT();
		wake_unlock(&fts_wq_data->touch_wake_lock);
        return 0;
    }
#endif
...
//<ASUS-BSP Robert_He 20170505> add esd check suspend/resume logic ++++++
	if (ESD_SUSPEND_TOUCH)
	{
		FTS_INFO("ESD check fail, should not resume touch");
		ESD_SUSPEND_TOUCH = false;
		return 0;
	}
//<ASUS-BSP Robert_He 20170505> add esd check suspend/resume logic ++++++
    if (!data->suspended) {
