...
#define W_REGISTER_2                            0x02
#define E_REGISTER_2                            0x08
#define C_REGISTER_2                            0x10
#define Z_REGISTER_5                            0x02
#define S_REGISTER_5                            0x40
#define V_REGISTER_6                            0x10
#define DCLICK_REGISTER_1                       0x10
#define SWIPE_REGISTER_1                        0x04
//<ASUS-BSP Robert_He 20170411> add gesture function  ------
                                 ...
                                 size_t count)
{
    ...
    if (FTS_SYSFS_ECHO_ON(buf)) {
        FTS_INFO("[GESTURE]enable dclick");
//<ASUS-BSP Robert_He 20170503> add gesture support for CN different property setting ++++
        fts_gesture_data.mode = ENABLE;
//<ASUS-BSP Robert_He 20170503> add gesture support for CN different property setting ----
		dclick_report_flag = true;
    }
    else if (FTS_SYSFS_ECHO_OFF(buf)) {
        FTS_INFO("[GESTURE]disable dclick");
        //fts_gesture_data.mode = DISABLE;
                                 ...
                                 size_t count)
{
    ...
    if (FTS_SYSFS_ECHO_ON(buf)) {
        FTS_INFO("[GESTURE]enable swipe");
        fts_gesture_data.mode = ENABLE;
		swipe_report_flag = true;
    }
    else if (FTS_SYSFS_ECHO_OFF(buf)) {
        FTS_INFO("[GESTURE]disable swipe");
        //fts_gesture_data.mode = DISABLE;
...
//<ASUSBSP Robert_He>20170622 just write open gesture to regiter without all 
    /* gesture not enable, return immediately */
    if (fts_gesture_data.mode == 0)
        return -EPERM;

	if (dclick_report_flag)
		register_d1 |= DCLICK_REGISTER_1;
	if (swipe_report_flag)
		register_d1 |= SWIPE_REGISTER_1;

	if (gesture_detect & GESTURE_SWITCH)
	{
		if (gesture_detect & W_SWITCH)
			register_d2 |= W_REGISTER_2;
		if (gesture_detect & E_SWITCH)
			register_d2 |= E_REGISTER_2;
		if (gesture_detect & C_SWITCH)
			register_d2 |= C_REGISTER_2;
		
		if (gesture_detect & S_SWITCH)
			register_d5 |= S_REGISTER_5;
		if (gesture_detect & Z_SWITCH)
			register_d5 |= Z_REGISTER_5;
		
		if (gesture_detect & V_SWITCH)
			register_d6 |= V_REGISTER_6;
	}
	
    for (i = 0; i < 5; i++) {
        fts_i2c_write_reg(i2c_client, FTS_REG_GESTURE_EN, 0x01);
        fts_i2c_write_reg(i2c_client, 0xd1, register_d1);
        fts_i2c_write_reg(i2c_client, 0xd2, register_d2);
        fts_i2c_write_reg(i2c_client, 0xd5, register_d5);
        fts_i2c_write_reg(i2c_client, 0xd6, register_d6);
