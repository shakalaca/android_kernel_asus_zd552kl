...
#define FTS_DEBUG(fmt, args...) printk(KERN_DEBUG "[FTS_TOUCH][%s]"fmt"\n", __func__, ##args)
#else
#define FTS_DEBUG(fmt, args...) printk(KERN_DEBUG "[FTS_TOUCH]"fmt"\n", ##args)
#endif

#define FTS_FUNC_ENTER() printk(KERN_INFO "[FTS_TOUCH]%s: Enter\n", __func__)
#define FTS_FUNC_EXIT()  printk(KERN_INFO "[FTS_TOUCH]%s: Exit(%d)\n", __func__, __LINE__)
#else
...
#define FTS_INFO(fmt, args...) do { \
            if (g_show_log) {printk(KERN_INFO "[FTS_TOUCH][Info]"fmt"\n", ##args);} \
        }  while (0)

#define FTS_ERROR(fmt, args...)  do { \
             if (g_show_log) {printk(KERN_ERR "[FTS_TOUCH][Error]"fmt"\n", ##args);} \
        }  while (0)
