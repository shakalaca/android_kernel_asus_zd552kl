...
//<ASUS-BSP Robert_He 20170429> update firmware verdor and version information after upgrade fw +++++
extern u8 asus_firmware_id_touch;
extern u8 asus_vendor_id_touch;
//<ASUS-BSP Robert_He 20170429> update firmware verdor and version information after upgrade fw -----
    ...
    if (bUpgradeFlag) {
        do {
            ...
            if (i_ret == 0) {   /* upgrade success */
                ...
                fts_i2c_read_reg(client, FTS_REG_FW_VER, &uc_tp_fm_ver);
//<ASUS-BSP Robert_He 20170429> update firmware verdor and version information after upgrade fw +++++
				fts_i2c_read_reg(client, FTS_REG_VENDOR_ID, &uc_tp_fw_vendor);
                FTS_DEBUG("[UPGRADE]: upgrade to new version 0x%x,new vendor 0x%x",
                          uc_tp_fm_ver,uc_tp_fw_vendor);
				asus_firmware_id_touch = uc_tp_fm_ver;
				asus_vendor_id_touch = uc_tp_fw_vendor;
//<ASUS-BSP Robert_He 20170429> update firmware verdor and version information after upgrade fw -----
            }
            else {              /* upgrade fail */
